...
after each step; replay reproduces the run deterministically, and `kinescope.diff_snapshots`
(also `kinescope diff <id> a b` on the CLI) shows the per-step state delta.
...
def agent() -> dict:
    ...
    for topic in ("comets", "tides"):
        ...
        kinescope.snapshot(state, f"after-{topic}")
    return state


def main() -> None:
    store = LocalStore(".kinescope")
    with kinescope.record("research", store=store) as rec:
        final = agent()
    run_id = rec.run_id
    ...
    for op in kinescope.diff_snapshots(store, run_id, 0, 1):
