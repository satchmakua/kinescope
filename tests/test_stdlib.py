...
def test_clock_rng_uuid_replay_identically(tmp_path):
    ...
    with eidetic.record("t", store=store, capture="all") as rec:
        a = draws()
...
def test_uuid_roundtrips_to_uuid_type(tmp_path):
    store = LocalStore(tmp_path / ".eidetic")
    with eidetic.record("t", store=store, capture=["uuid"]) as rec:
        u1 = uuid.uuid4()
    with eidetic.replay(rec.run_id, store=store) as rep:
        u2 = uuid.uuid4()
...
def test_capture_is_off_by_default(tmp_path):
    store = LocalStore(tmp_path / ".eidetic")
    with eidetic.record("t", store=store) as rec:  # no capture requested
        time.time()
        random.random()
