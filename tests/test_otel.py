...
carrying the `gen_ai.*` attributes, under one `eidetic.run` parent — verified offline with
...
def test_export_otel_emits_gen_ai_spans(tmp_path):
    ...
    n = eidetic.export_otel(rec.run_id, store=store, tracer_provider=provider)
    provider.force_flush()
    spans = {s.name: s for s in exporter.get_finished_spans()}

    assert n == 2  # one LLM span + one tool span (the run parent isn't counted)
    assert "eidetic.run" in spans
...
def test_export_otel_uses_recorded_timing(tmp_path):
    ...
    exporter = InMemorySpanExporter()
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    eidetic.export_otel(rec.run_id, store=store, tracer_provider=provider)
    provider.force_flush()
