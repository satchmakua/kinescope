...
Name: **Eidetic** (eidetic memory = perfect total recall). Working codename — rename freely; the word is generic English and carries no usable trademark for dev tools, but do a quick PyPI/name check before publishing the package.

> **Privacy / secrets note (read first).** Eidetic records the full request/response of every LLM and tool call, which routinely contains API keys, auth headers, user PII, and proprietary prompts. Redaction is therefore a *core feature, not an afterthought*: Eidetic strips credential headers (`authorization`, `x-api-key`, `anthropic-*` auth) **by default** and exposes a scrubber hook for payload fields. Traces are local files by default; nothing leaves the machine unless the user exports it. See §6.6.
...
Eidetic is the systems answer. It sits at the **nondeterministic frontier** of an agent — the handful of places where the same code can produce different results: LLM calls, tool calls, the clock, the RNG/UUIDs, retrieval — and records the *output* of every crossing into an ordered event log plus periodic state snapshots. From that trace it can:
...
The thing a reader should "see": an agent fails a task; you `eidetic ui` the recorded run, scrub to step 7 where it called the wrong tool, hit **fork**, override that one tool result, and watch the branched run complete successfully — fully reproducible from the trace. That fork-and-fix is the entire pitch.
...
- **Local-first, low-friction**: the recording core has a tiny dependency footprint (essentially `httpx`); storage is a `.eidetic/` dir (SQLite index + content-addressed blobs) with no external services.
...
- **Automatic root-cause analysis.** Eidetic gives you the deterministic substrate and the fork; it does not diagnose for you (an LLM-judge "explain this divergence" feature is a stretch).
...
- **Non-HTTP tool nondeterminism we can't see.** Tools that read files, hit databases via non-HTTP drivers, or spawn subprocesses are captured only via the `@eidetic.tool` wrapper; un-wrapped side effects are surfaced by the divergence detector, not magically captured.
...
| CLI | **Typer** (`eidetic[cli]` extra) | Clean, typed, Click-based; behind an extra so the core stays minimal. |
| TUI | **Textual 8.2.7** (`eidetic[tui]` extra) | Modern, well-maintained terminal UI; gives the scrubbable timeline + fork demo without a web stack. |
| Pretty output | **Rich** (pulled by Typer/Textual) | Diff coloring, tables in `show`/`diff`. |
| Packaging | **`uv` + `hatchling`**, `pyproject.toml` | Fast, modern, lockfile-based; `eidetic` console entry point. |
| Test / lint | **pytest**, **ruff**, **mypy** | Standard. Replay determinism is itself the headline test target. |
| Storage adapter (later) | **`pymongo`** (`eidetic[mongo]` extra) | Document-shaped traces map cleanly to Mongo; ships post-v1 as a `MongoStore` implementing `TraceStore`. |

**Dependency story (the adoption lever):** the *core* engine depends on **`httpx` only**. `anthropic`, `typer`+`rich`, `textual`, and `pymongo` are all **extras**. `pip install eidetic` gives you record/replay; `pip install eidetic[anthropic,cli,tui]` gives the full experience. All licenses are permissive/commercial-friendly (anthropic MIT, httpx BSD, Textual MIT, Typer MIT, Rich MIT, pymongo Apache-2.0).
...
class Run:
    ...
    sdk_versions: dict            # {"eidetic": "...", "anthropic": "0.112.0", "python": "3.11.x"}
...
```python
class EideticTransport(httpx.BaseTransport):           # + async sibling
...
eidetic/
  src/eidetic/
    ...
    intercept/
      http.py          # EideticTransport (sync+async) for LLM/retrieval
...
- **LLM / retrieval (HTTP):** `session.http_client()` returns an `httpx.Client`/`AsyncClient` whose transport is `EideticTransport` (§4.4). The user passes it to `anthropic.Anthropic(http_client=…)`. A convenience `eidetic.instrument(client)` swaps the transport on an already-built client for the zero-reconstruction path.
- **Tools:** `@eidetic.tool` wraps a callable; `eidetic.instrument_tools(mapping)` wraps a name→callable registry in place. Records `(name, canon args)` → return/raise.
...
```python
with eidetic.record("nightly-repro", store=eidetic.LocalStore(".eidetic")) as ses:
...
Policy via `record()/replay(policy=…)`: `strict` (raise on first), `warn` (default — log, continue using recorded-by-position), `off`. Divergences are written to `Run.divergences` and shown in `show`/TUI. **This is the feature that lets Eidetic claim determinism honestly:** it tells you precisely where the guarantee leaked.

### 6.5 State snapshots & diffs

- **Capture:** `eidetic.snapshot(obj, label=None)` serializes a document-shaped state (the messages list, scratchpad, memory — user's choice) to canonical JSON and stores it content-addressed, tagged `after_seq = current seq`. Optionally `record(snapshot=lambda: agent.state)` auto-snapshots after each `llm` event.
...
`LocalStore` layout (everything under `.eidetic/`):
...
```
eidetic record -- python weather_agent.py     # run an agent under recording
eidetic ls                                     # list runs (id, label, status, #events, divergences)
eidetic show <run-id> [--step k]               # event detail: messages / tool I/O / meta
eidetic diff <run-id> <a> <b>                  # state diff between two steps
eidetic replay <run-id> [--policy strict]      # deterministic replay + divergence report
eidetic fork <run-id> --at k --override-tool '{"temp_f":71}'   # → new child run id
eidetic ui <run-id>                            # Textual timeline (scrub · inspect · fork)
```

`record -- <cmd>` runs the user's program with an env flag that auto-activates a default session (for programs that call `eidetic.record()` themselves, it's a no-op wrapper). The first-class path is the in-code `with eidetic.record(...)`.
...
```
┌ Eidetic ── run 01JZ8…-a3f1  "weather repro"  ⚠ 0 divergences ──────────────────────┐
...
- **M0 — Walking skeleton (record→replay one call).** `with eidetic.record()` + `EideticTransport` capture a single non-streaming Anthropic `messages.create` to `LocalStore`; `eidetic replay <id>` re-runs a one-call example agent returning the recorded completion; `eidetic ls`/`show`. **Proves** the end-to-end record→replay loop and the store on the simplest agent.
- **M1 — Full frontier + ordering + divergence.** Add `@eidetic.tool`, clock/RNG/UUID monkeypatch, the global `seq`, **async** + **SSE streaming** capture, and the divergence detector. Deterministically replay a multi-step, tool-using agent. **Proves** pillar 1 (total interception) and honest determinism.
- **M2 — State snapshots & diffs.** `eidetic.snapshot()`, content-addressed dedup, lazy RFC 6902 diffs; `show --step k`, `diff a b`. **Proves** pillar 3 (state diffing) and makes runs inspectable.
- **M3 — Branching (the novel hook).** `eidetic fork --at k --override …`, replay→live switch, child-run lineage, re-recorded tail. **Proves** pillar 2 — the counterfactual that makes Eidetic more than a tracer.
...
- **Open question — auto-snapshot ergonomics.** Is a single `eidetic.snapshot(state)` call enough, or do we need framework-aware auto-capture of the messages list? Resolve in M2 against the real example agent.
...
- **rr** (Mozilla) & **Pernosco** — deterministic record-replay for native processes; the lineage and the bar for "replay any run." Eidetic is this idea, moved to the LLM-agent boundary.
- **VCR.py** (vcrpy 8.x) — HTTP record/replay via "cassettes." Eidetic generalizes the *technique* (transport-level record/replay) but adds agent state, global ordering across non-HTTP boundaries, and counterfactual branching — none of which VCR targets.
- **LangGraph time-travel / checkpointers** — fork/replay of agent runs, but **framework-locked** and rewinds *graph state* rather than replaying recorded *nondeterministic inputs*. Eidetic works *under* any framework at the SDK/HTTP seam.
- **Agent observability** (LangSmith, Langfuse, AgentOps, Arize Phoenix, W&B Weave, OpenLLMetry/Traceloop, Laminar) — strong *tracing/visualization*, but read-only: no deterministic replay, no single-event counterfactual fork. Eidetic's unoccupied niche.
...
- **Stack reuse note:** the `MongoStore` adapter ties Eidetic to a document-DB stack if/when the author wants persistence beyond local files; the `TraceStore` port keeps that a drop-in, not a rewrite.
```
